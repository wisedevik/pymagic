...
class CSVTable:
    def __init__(self, node, size: int) -> None:
        self._column_name_list: LogicArrayList[str] = LogicArrayList[str]()
        self._column_list: LogicArrayList[CSVColumn] = LogicArrayList[CSVColumn]()
        self._row_list: LogicArrayList[CSVRow] = LogicArrayList[CSVRow]()

        self._size = size
        self._node = node

    def add_column(self, name: str) -> None:
        self._column_name_list.add(name)
    ...
    def add_and_convert_value(self, value: str, index: int) -> None:
        column = self._column_list[index]
                ...
                case 2:
                    ...
                    if success:
                        ...
                        Debugger.warning(
                            ...
                            f"in Boolean column '{self._column_name_list[index]}', "
    ...
    def get_column_name(self, idx: int) -> str:
        return self._column_name_list[idx]

    def add_column_type(self, type: int) -> None:
        self._column_list.add(CSVColumn(type, self._size))

    def get_column_count(self):
        return self._column_list.count

    def get_value_at(self, column_index: int, index: int) -> str:
        if column_index >= 0:
            return self._column_list[column_index].get_string_value(index)
        return ""

    def get_value(self, name: str, index: int) -> str:
        return self.get_value_at(self._column_name_list.index_of(name), index)

    def get_column_index_by_name(self, name: str) -> int:
        return self._column_name_list.index_of(name)

    def get_integer_value_at(self, column_index: int, index: int) -> int:
        if column_index >= 0:
            return self._column_list[column_index].get_int_value(index)
        return 0
    ...
    def get_boolean_value_at(self, column_index: int, index: int) -> bool:
        if column_index >= 0:
            return self._column_list[column_index].get_boolean_value(index)
        return False
    ...
    def add_row(self, row: CSVRow) -> None:
        self._row_list.add(row)

    def get_column_row_count(self) -> int:
        return self._column_list[0].get_size()

    def get_row_count_of_csv_row(self, row: CSVRow) -> int:
        return -1

    def get_row_count(self) -> int:
        return self._row_list.count

    def get_array_size_at(self, row: CSVRow, index: int):
        if self._row_list.count > 0:
            row_index = self._row_list.index_of(row)

            if row_index != -1:
                column = self._column_list[index]
                return column.get_array_size(
                    self._row_list[row_index].get_row_offset(),
                    (
                        column.get_size()
                        if row_index + 1 >= self._row_list.count
                        else self._row_list[row_index + 1].get_row_offset()
                    ),
                )
        return 0

    def create_row(self) -> None:
        self._row_list.add(CSVRow(self))

    def column_names_loaded(self) -> None:
        self._column_list.ensure_capacity(self._column_name_list.count)

    def validate_column_types(self) -> None:
        if self._column_name_list.count != self._column_list.count:
            Debugger.warning(
                f"Column name count {self._column_name_list.count}, "
                f"column type count {self._column_list.count}, "
